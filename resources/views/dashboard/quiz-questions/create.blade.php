                ...
                <form action="{{ route('dashboard.quiz-questions.store', $quizCompetition) }}" method="POST"
                    ...
                    <div class="form-group" id="groupsCountGroup" style="display:none;">
                        ...
                        <input type="number" class="form-control @error('groups_count') is-invalid @enderror"
                            id="groups_count" name="groups_count" value="{{ old('groups_count') }}" min="0">
                        @error('groups_count')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                            ...
                            @for($i = 0; $i < 4; $i++)
                                <div class="input-group mb-2 choice-row">
                                    ...
                                    <input type="text" class="form-control choice-group-name" name="choices[{{ $i }}][group_name]"
                                        placeholder="اسم المجموعة (للسحب والإفلات)" value="{{ old("choices.{$i}.group_name") }}" style="display:none; max-width: 25%;">
        ...
        $(document).ready(function () {
            ...
            function toggleChoices() {
                ...
                if (isOrdering) {
                    ...
                    hint.innerHTML = 'أدخل كلمات الاختيار وحدد الكلمة <strong>الصحيحة ✓</strong>. ضع <code>___</code> في نص السؤال مكان الفراغ.';
                } else {
