...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <div>
            <h1 class="h3 mb-2 text-gray-800">
                <i class="fas fa-plus-circle text-primary mr-2"></i>إضافة سجل مشي
            </h1>
        </div>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body">
            <form action="{{ route('dashboard.walking.store') }}" method="POST">
                @csrf
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" name="steps" class="form-control @error('steps') is-invalid @enderror"
                                   value="{{ old('steps') }}" min="0" required>
                            @error('steps')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" name="distance_km" class="form-control @error('distance_km') is-invalid @enderror"
                                   value="{{ old('distance_km') }}" min="0" step="0.01" placeholder="0">
                            @error('distance_km')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" name="duration_minutes" class="form-control @error('duration_minutes') is-invalid @enderror"
                                   value="{{ old('duration_minutes') }}" min="0" placeholder="0">
                ...
                <div class="form-group">
                    <label>ملاحظات</label>
                    <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3" placeholder="ملاحظات اختيارية">{{ old('notes') }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save mr-2"></i>حفظ السجل
                    </button>
                    <a href="{{ route('dashboard.walking.index') }}" class="btn btn-secondary">
                        إلغاء
                    </a>
