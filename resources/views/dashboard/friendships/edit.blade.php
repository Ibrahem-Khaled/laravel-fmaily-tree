...
@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <h1 class="h3 mb-0 text-gray-800">تعديل الصداقة</h1>
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('friendships.update', $friendship) }}" method="POST">
                    @csrf
                    @method('PUT')
                                ...
                                <select id="person_id" name="person_id"
                                    ...
                                    @foreach($persons as $person)
                                        <option value="{{ $person->id }}"
                                                {{ old('person_id', $friendship->person_id) == $person->id ? 'selected' : '' }}>
                                            {{ $person->full_name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select id="friend_id" name="friend_id"
                                    ...
                                    @foreach($persons as $person)
                                        <option value="{{ $person->id }}"
                                                {{ old('friend_id', $friendship->friend_id) == $person->id ? 'selected' : '' }}>
                                            {{ $person->full_name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description"
                                  ...
                                  rows="3"
                                  placeholder="وصف مختصر عن الصداقة...">{{ old('description', $friendship->description) }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="friendship_story" name="friendship_story"
                                  ...
                                  rows="5"
                                  placeholder="قصة أو تفاصيل عن الصداقة...">{{ old('friendship_story', $friendship->friendship_story) }}</textarea>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            // حفظ القيم الأصلية
            $('#person_id, #friend_id').each(function() {
                $(this).data('original-value', $(this).val());
            });
