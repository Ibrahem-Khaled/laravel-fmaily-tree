...
<body>
        ...
        <div class="modal-dialog modal-dialog-centered modal-fullscreen">
            <div class="modal-content">
                ...
                <div class="zoom-controls">
                    <button id="zoomOutBtn" class="btn btn-outline-light rounded-circle mx-1"><i class="fas fa-search-minus"></i></button>
                    <button id="zoomInBtn" class="btn btn-outline-light rounded-circle mx-1"><i class="fas fa-search-plus"></i></button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
     document.addEventListener('DOMContentLoaded', () => {
         // ... (باقي تعريفات المتغيرات والدوال تبقى كما هي)
         ...
         function createPersonNode(person, level = 0) { const hasChildren = person.children_count > 0; const uniqueId = `person_${person.id}_level_${level}`; const itemClass = (level === 0) ? 'accordion-group-item' : 'accordion-item'; const parentSelector = `#tree_level_${level}`; const hasPhoto = person.photo_url; const bgClass = hasPhoto ? 'photo-bg' : ''; const bgStyle = hasPhoto ? `style="background-image: url('${person.photo_url}')"` : ''; const buttonContent = `${createPhoto(person, 'md')}<span class="person-name">${person.first_name}</span>`; const buttonOrDiv = hasChildren ? `<button class="accordion-button collapsed ${bgClass}" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_${uniqueId}" onclick="loadChildren(this)" data-person-id="${person.id}" data-level="${level + 1}" ${bgStyle}>${buttonContent}</button>` : `<div class="accordion-button collapsed ${bgClass}" ${bgStyle}>${buttonContent}</div>`; return `<div class="${itemClass}"><h2 class="accordion-header">${buttonOrDiv}</h2><div class="actions-bar"><button class="btn" onclick="showPersonDetails(${person.id})"><i class="fas fa-info-circle me-1"></i> التفاصيل</button></div>${hasChildren ? `<div id="collapse_${uniqueId}" class="accordion-collapse collapse" data-bs-parent="${parentSelector}"><div class="accordion-body p-0"><div class="accordion" id="tree_level_${level + 1}"></div></div></div>` : ''}</div>`; }
         window.loadChildren = async (buttonElement) => { if (buttonElement.dataset.loaded === 'true') return; const personId = buttonElement.dataset.personId; const level = parseInt(buttonElement.dataset.level); const childrenContainer = document.querySelector(`${buttonElement.dataset.bsTarget} .accordion`); if (!childrenContainer) return; childrenContainer.innerHTML = `<div class="p-2 text-center text-muted small">جاري التحميل...</div>`; const data = await fetchAPI(`/person/${personId}/children`); childrenContainer.innerHTML = ''; if (data && data.children && data.children.length > 0) { data.children.forEach(child => { childrenContainer.innerHTML += createPersonNode(child, level); }); } else { childrenContainer.innerHTML = `<div class="p-2 text-center text-muted small">لا يوجد أبناء.</div>`; } buttonElement.dataset.loaded = 'true'; };
         ...
         async function loadInitialTree() { const data = await fetchAPI('/family-tree'); if (data && data.tree && data.tree.length > 0) { treeContainer.innerHTML = ''; data.tree.forEach(person => { treeContainer.innerHTML += createPersonNode(person, 0); }); } else { treeContainer.innerHTML = '<div class="alert alert-warning text-center">لا توجد بيانات لعرضها في تواصل العائلة.</div>'; } }
        ...
        // ⭐⭐⭐ الحل لمشكلة التداخل وسهولة الاستخدام ⭐⭐⭐
        // هذا الكود يضمن إغلاق أي فرع مفتوح قبل فتح فرع جديد
        document.addEventListener('show.bs.collapse', function (event) {
         ...
         // هذا الكود مسؤول عن التمرير الأفقي الناعم عند فتح جيل جديد
