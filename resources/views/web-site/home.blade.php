...
@section('content')
    ...
    <style>
        ...
        .il-section-card:hover {
            ...
            box-shadow: 0 16px 40px rgba(15,23,42,0.1), 0 0 0 1px rgba(34,197,94,0.12), 0 8px 30px rgba(34,197,94,0.1);
        }
        .il-section-card-img {
            aspect-ratio: 1;
        ...
        .il-scrollbar-home::-webkit-scrollbar { width: 6px; }
...
<div class="flex items-center justify-between flex-wrap gap-2">
            ...
            @if(session('success') && request()->is('/'))
                <div class="mb-4 p-3 md:p-4 rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-900 text-right text-sm">
                    <i class="fas fa-check-circle text-emerald-600 ml-2"></i>{{ session('success') }}
                </div>
            @endif

            {{-- Cards Grid --}}
            <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-2 md:gap-3" id="ilLinksGrid">
                @foreach($importantLinks as $ilLink)
                    <div class="il-section-card group"
                         data-il-id="{{ $ilLink->id }}"
                         onclick="openIlDetailModal({{ $ilLink->id }})"
                         role="button" tabindex="0"
                         onkeydown="if(event.key==='Enter')openIlDetailModal({{ $ilLink->id }})">
                        <div class="il-section-card-img">
                            <img src="{{ $ilLink->image_url }}"
                                 alt="{{ $ilLink->title }}"
    ...
    {{-- Detail Modal --}}
    <div id="ilDetailModal" class="fixed inset-0 z-50 hidden overflow-y-auto il-scrollbar-home il-section-modal" aria-modal="true">
        <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm" onclick="closeIlDetailModal()"></div>
        <div class="flex min-h-full items-center justify-center p-4 lg:p-10">
            <div class="relative bg-white rounded-2xl w-full max-w-lg lg:max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-gray-100">
                <button type="button" onclick="closeIlDetailModal()"
                    class="absolute left-4 top-4 z-20 p-2.5 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-200 transition-colors" aria-label="إغلاق">
    ...
    {{-- Suggest Modal --}}
    <div id="ilSuggestModal" class="fixed inset-0 z-50 hidden overflow-y-auto il-scrollbar-home il-section-modal" aria-modal="true">
        <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm" onclick="closeIlSuggestModal()"></div>
        <div class="flex min-h-full items-center justify-center p-4 lg:p-10">
            <div class="relative bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto il-scrollbar-home p-5 lg:p-8 shadow-2xl border border-gray-100">
                <button type="button" onclick="closeIlSuggestModal()"
                    class="absolute left-4 top-4 p-2.5 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-200 transition-colors">
                    <i class="fas fa-times"></i>
                </button>
                <h3 class="text-xl lg:text-2xl font-bold text-emerald-700 mb-1 text-right">اقتراح تطبيق</h3>
                <p class="text-gray-500 text-sm mb-5 text-right">بعد المراجعة قد يظهر اقتراحك للجميع.</p>
                <form action="{{ route('important-links.suggest') }}" method="POST" enctype="multipart/form-data" class="space-y-4" dir="rtl">
                    @csrf
                    @if(!auth()->check())
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">اسمك <span class="text-red-600">*</span></label>
                            <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-3 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-1">رقم هاتفك <span class="text-red-600">*</span></label>
                            <input type="text" name="phone" value="{{ old('phone') }}" required class="w-full px-3 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-900 focus:ring-2 focus:ring-emerald-500 text-sm">
                        </div>
                    @endif
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">العنوان <span class="text-red-600">*</span></label>
                        <input type="text" name="title" value="{{ old('title') }}" required class="w-full px-3 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-900 focus:ring-2 focus:ring-emerald-500 text-sm">
                    </div>
                    <p class="text-xs text-gray-600">أدخل <span class="text-emerald-700 font-bold">رابطاً واحداً على الأقل</span> من الخيارات التالية.</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">رابط عام</label>
                            <input type="url" name="url" value="{{ old('url') }}" placeholder="https://..." class="w-full px-3 py-2 rounded-xl bg-white border border-gray-200 text-gray-900 focus:ring-2 focus:ring-emerald-500 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-gray-700 mb-1">رابط iOS (App Store)</label>
                            <input type="url" name="url_ios" value="{{ old('url_ios') }}" placeholder="App Store" class="w-full px-3 py-2 rounded-xl bg-white border border-gray-200 text-gray-900 focus:ring-2 focus:ring-emerald-500 text-sm">
                        </div>
                        <div class="sm:col-span-2">
                            <label class="block text-xs font-bold text-gray-700 mb-1">رابط أندرويد (Google Play)</label>
                            <input type="url" name="url_android" value="{{ old('url_android') }}" placeholder="Google Play" class="w-full px-3 py-2 rounded-xl bg-white border border-gray-200 text-gray-900 focus:ring-2 focus:ring-emerald-500 text-sm">
                        </div>
                    </div>
                    <input type="hidden" name="type" value="app">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">الوصف</label>
                        <textarea name="description" rows="3" class="w-full px-3 py-2 rounded-xl bg-white border border-gray-200 text-gray-900 focus:ring-2 focus:ring-emerald-500 text-sm">{{ old('description') }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">صورة غلاف</label>
                        <input type="file" name="image" accept="image/*" class="w-full text-sm text-gray-600 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:bg-emerald-600 file:text-white">
                    </div>
                    <button type="submit" class="w-full bg-gradient-to-r from-emerald-500 to-green-600 py-3 rounded-xl text-white font-bold shadow-md hover:shadow-lg transition-all">
                        إرسال الاقتراح
                    </button>
                </form>
            </div>
    ...
    (function(){
        ...
        window.openIlDetailModal = function(id) {
            ...
            const btnBase = 'inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl font-bold text-white shadow-md transition-all hover:scale-[1.02] hover:shadow-lg text-sm';

            const media = Array.isArray(link.media) ? link.media : [];
            let mediaHtml = '';
            if (media.length > 0) {
                media.forEach(function(m) {
                    const t = m.title ? '<p class="text-sm font-bold text-gray-900 mt-3">' + escIl(m.title) + '</p>' : '';
                    const d = m.description ? '<p class="text-xs text-gray-600 mt-1 leading-relaxed">' + escIl(m.description) + '</p>' : '';
                    if (m.kind === 'video') {
                        mediaHtml += '<div class="mb-6 last:mb-0 rounded-xl overflow-hidden border border-gray-200 bg-gray-900"><video controls class="w-full max-h-80" src="' + escIl(m.file_url || '') + '"></video></div>' + t + d;
                    } else {
                        mediaHtml += '<div class="mb-6 last:mb-0 rounded-xl overflow-hidden border border-gray-200 bg-gray-50"><img src="' + escIl(m.file_url || '') + '" alt="" class="w-full object-contain max-h-72 mx-auto" loading="lazy"></div>' + t + d;
                    }
                });
            } else {
                mediaHtml = '<div class="rounded-xl overflow-hidden border border-gray-200 bg-gray-50 mb-4"><img src="' + escIl(link.image_url) + '" alt="" class="w-full object-contain max-h-60 mx-auto py-4" onerror="this.style.display=\'none\'"></div>';
            }

            let actionsHtml = '';
            if (isApp) {
                const parts = [];
                if (link.url_ios) parts.push('<a href="' + escIl(link.url_ios) + '"' + newTab + ' class="' + btnBase + ' bg-gradient-to-br from-gray-700 to-gray-900"><i class="fab fa-apple text-base"></i> App Store</a>');
                if (link.url_android) parts.push('<a href="' + escIl(link.url_android) + '"' + newTab + ' class="' + btnBase + ' bg-gradient-to-br from-emerald-600 to-green-800"><i class="fab fa-google-play"></i> Google Play</a>');
