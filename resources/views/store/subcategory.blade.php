...
<head>
    ...
    <title>{{ $subcategory->name }} - متجر الأسر المنتجة</title>
...
<body class="text-gray-800 relative overflow-x-hidden">
    ...
    <div class="container mx-auto px-4 py-4 lg:py-8 relative z-10">
        ...
        <nav class="mb-6 fade-in-up">
            <ol class="flex items-center space-x-2 space-x-reverse text-sm">
                ...
                @if($subcategory->category)
                    <li><i class="fas fa-chevron-left text-gray-400"></i></li>
                    <li><a href="{{ route('store.category', $subcategory->category) }}" class="text-green-600 hover:text-green-700">{{ $subcategory->category->name }}</a></li>
                @endif
                <li><i class="fas fa-chevron-left text-gray-400"></i></li>
                <li class="text-gray-600">{{ $subcategory->name }}</li>
            </ol>
        </nav>
        ...
        <div class="text-center mb-8 fade-in-up">
            ...
            @endif
            <h1 class="text-4xl lg:text-5xl font-black gradient-text mb-4">{{ $subcategory->name }}</h1>
            @if($subcategory->description)
                <p class="text-lg text-gray-600 max-w-2xl mx-auto">{{ $subcategory->description }}</p>
            @endif
        </div>
        ...
        <div class="fade-in-up">
            ...
            @else
                <div class="glass-effect rounded-3xl p-12 text-center">
                    ...
                    <h3 class="text-2xl font-bold gradient-text mb-3">لا توجد منتجات في هذه الفئة الفرعية حالياً</h3>
                </div>
            @endif
        </div>
    </div>
