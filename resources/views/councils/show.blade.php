@php
    ...
    $galleryPaths = $council->images->map(fn($img) => $img->image_url)->values();
@endphp
<!DOCTYPE html>
...
<head>
    ...
    <title>{{ $council->name }} - مجالس عائلة السريع</title>
    ...
    <style>
        ...
        .council-description b {
            font-weight: 700;
            color: #059669;
        }

        .council-description em,
        .council-description i {
            font-style: italic;
        }
            ...
            <header class="relative">
                @if ($council->images->count() > 0)
                    <div class="relative h-60 sm:h-72 lg:h-80 overflow-hidden">
                        <img src="{{ $council->images->first()->image_url }}" alt="{{ $council->name }}" class="w-full h-full object-cover">
                                    ...
                                    @if($council->address)
                                        <div class="flex items-start gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-gray-700">{{ $council->address }}</p>
                                            </div>
                                        </div>
                                    @endif
                                    @if($council->working_days_from && $council->working_days_to)
                                        <div class="flex items-center gap-2">
                                            <i class="fas fa-calendar-alt text-emerald-500"></i>
                                            <span>أيام العمل:</span>
                                            <span class="font-medium">{{ $council->working_days_from }} - {{ $council->working_days_to }}</span>
                                        </div>
                                    @endif
                                    @if($council->images->count() > 0)
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span>عدد الصور:</span>
                                            <span class="font-medium">{{ $council->images->count() }}</span>
                                        </div>
                                    @endif
            ...
            <div class="p-6 lg:p-10 space-y-14">
                @if($council->images->count() > 0)
                    <section>
                        <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
                            <div>
                                <h2 class="text-2xl lg:text-3xl font-bold font-serif bg-gradient-to-r from-emerald-600 to-teal-500 text-transparent bg-clip-text">معرض الصور</h2>
                                <p class="text-sm text-gray-500 mt-1">صور من مجلس {{ $council->name }}</p>
                            </div>
                            <span class="px-3 py-1 bg-emerald-50 text-emerald-600 text-sm rounded-full border border-emerald-100">
                                {{ $council->images->count() }} صورة
                            </span>
                        </div>
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
                            @foreach($council->images as $index => $image)
                                <div class="group relative overflow-hidden rounded-2xl border border-white/60 shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer"
                                    ...
                                    <img src="{{ $image->image_url }}" alt="{{ $image->description ?? $council->name }}" class="w-full h-48 md:h-52 object-cover transition-transform duration-500 group-hover:scale-105">
                                    ...
                                    @if($image->description)
                                        <div class="absolute bottom-3 right-3 left-3 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                            <h3 class="font-semibold text-base line-clamp-1">{{ Str::limit($image->description, 50) }}</h3>
                                        </div>
                                    @endif
                                </div>
                            @endforeach
                ...
                @if($council->google_map_url)
                    <section>
                        <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
                            <div>
                                <h2 class="text-2xl lg:text-3xl font-bold font-serif bg-gradient-to-r from-emerald-600 to-teal-500 text-transparent bg-clip-text">الموقع على الخريطة</h2>
                                <p class="text-sm text-gray-500 mt-1">موقع مجلس {{ $council->name }} على الخريطة</p>
                            </div>
                        </div>
                        <div class="bg-white border border-emerald-100 rounded-2xl p-6 shadow-md">
                            <a href="{{ $council->google_map_url }}" target="_blank"
                               class="inline-flex items-center gap-3 px-6 py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl">
                                <i class="fas fa-map-marked-alt text-xl"></i>
                                <div>
                                    <div class="font-semibold">فتح في جوجل ماب</div>
                                    <div class="text-sm opacity-90">عرض الموقع على الخريطة</div>
                                </div>
                                <i class="fas fa-external-link-alt mr-auto"></i>
                            </a>
                        </div>
                    </section>
                @endif

                @if($council->images->isEmpty() && !$council->google_map_url)
                    <div class="bg-white/90 backdrop-blur-md border border-white/60 rounded-3xl p-10 text-center shadow-lg">
                        ...
                        <p class="text-lg text-gray-600">لم يتم إضافة محتوى تفصيلي لهذا المجلس بعد. ترقبوا تحديثات قادمة قريباً.</p>
                    </div>
                @endif
            </div>
        </article>
